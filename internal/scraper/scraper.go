...
import (
	...
	"TripAdvisor-Scraper-API/internal/scraper/internal/config"
	"TripAdvisor-Scraper-API/internal/scraper/pkg/tripadvisor"
	"TripAdvisor-Scraper-API/internal/scraper/pkg/utils"
)
...
func Scraper(url string) string {
	...
	resultData, err := tripadvisor.ReviewsData(allReviews, location)
	if err != nil {
		log.Fatalf("Error converting reviews data to JSON: %v", err)
	}
	// log.Printf("Result Data: %s", resultData)
